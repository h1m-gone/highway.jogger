...
func _physics_process(delta: float) -> void:
	...
	$"ene2/AnimatedSprite2D".play("jum")
...
func finishboom() -> void:
	...
	if $"ene2/AnimatedSprite2D2".animation =="1":
		$"ene2".position.x=1200
		d=true
...
func ene_move() -> void:
	if $"ene".position.y>534 and $"ene".position.y<614:
		...
		$"ene".position.y-=40
	if $"ene2".position.y>534 and $"ene2".position.y<614:
		if randi_range(1,2) == 1:
			$"ene2".position.y+=10
		else:
			$"ene2".position.y-=10
	elif $"ene2".position.y==534:
		$"ene2".position.y+=30
	elif $"ene2".position.y==614:
		$"ene2".position.y-=40
	$Timer2.start()
